            ...
            try {
                ...
                if (isEditing && filePath) {
                    ...
                    if (match) {
                        ...
                        setPost({
                            title: extract('title'), slug: loadedSlug,
                        ...
                        setPost(p => ({ ...p, content: String(marked.parse(text)), slug: loadedSlug }));
                    }
                }
            } catch (err: any) {
    ...
    const handleSave = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditing && post.slug !== originalSlug) {
                // Renomeando: escreve no novo caminho e deleta o antigo
                await githubApi('write', targetPath, { content: markdown, message: `CMS: Renomeando artigo de ${originalSlug} para ${post.slug}` });
                if (filePath) {
                    await githubApi('delete', filePath, { sha: fileSha, message: `CMS: Apagando slug antigo do artigo ${originalSlug} após renomeação` });
                }
                
                triggerToast('Artigo renomeado e salvo com sucesso!', 'success', 100);
                setPendingUploads({});
                
                setTimeout(() => {
                    window.location.href = `/admin/posts/edit?file=${encodeURIComponent(targetPath)}&type=post`;
                }, 1500);
            } else {
                // Sobrescrevendo ou criando novo
